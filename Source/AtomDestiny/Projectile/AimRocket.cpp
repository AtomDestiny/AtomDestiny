...
void AAimRocket::Tick(float deltaSeconds)
{
    ...
    m_trace->SetActorRotation(FQuat::FindBetweenVectors(m_rocketEngine->GetForwardVector(), FVector::ForwardVector));
}
...
void AAimRocket::OnDisabled()
{
    if (m_trace.IsValid())
    {
        AtomDestiny::ObjectPool::Instance().Despawn(m_trace);
    }
